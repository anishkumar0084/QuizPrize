...
public class ProfileActivity extends AppCompatActivity {
    ...
    ImageView tvLogout;
    int REQUEST_IMAGE_CAPTURE = 100, REQUEST_CROP_IMAGE = 120, SELECT_FILE = 110, reqWritePermission = 1;
    File output = null;
    Uri imageUri;
    ...
    public void SelectProfileImage() {
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED
            ...
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_MEDIA_IMAGES, Manifest.permission.CAMERA}, 0);
        }
        else
        {
            selectDialog();
...
//                CropImage.ActivityResult result = CropImage.getActivityResult(data);
                CropImage.ActivityResult result=CropImage.getActivityResult(data);
                if (result != null) {
                    Uri resultUri = Uri.parse(result.toString());

                    // Use getUriContent instead of getUri
                    filePath = getRealPathFromURI(resultUri);
                    if (filePath != null) {
                        uploadFileWithVolley(filePath);
                    } else {
                        // Handle error if file path is null
                        Log.e("File Path Error", "Could not get the file path.");
                    }
                }
            }
        }
    ...
    public String getRealPathFromURI(Uri uri) {
        ...
        if (result == null) {
            result = uri.getPath(); // Fallback to URI path if the query fails
        }
        return result;
    }
